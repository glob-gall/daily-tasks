...
export default function UpdateTask() {
  ...
  const defaultValues: TaskFormFields = task
    ? {
        color:
          colorOptions.find((c) => c.color === task.color) ||
          defaultTaskFormValues.color,
        dailyColor:
          colorOptions.find((c) => c.color === task.color) ||
          defaultTaskFormValues.dailyColor,
  ...
  const handleSubmitForm = useCallback(
    (values: TaskFormFields) => {
      ...
      if (values.type === "daily") {
        ...
        if (values.dailyTime && values.dailyTime !== ":")
          dto.time = values.dailyTime;
  ...
  const handleCancel = useCallback(() => {
    ...
    Alert.alert("DELETAR META", "Clique em OK para deletar sua meta.", [
      { text: "Cancelar" },
  ...
  return (
    <Base>
      <Margin>
        <TaskForm errors={errors} control={control} task={task} />
